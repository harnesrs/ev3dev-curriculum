...
Authors: David Fisher and Tony Kaiser.
"""  # done PUT YOUR NAME IN THE ABOVE LINE.

# done. Copy the contents of your m1_drive_timed.py and paste that text into this file below these comments.
...
# done. Instead of using the run_forever, time.sleep, stop pattern switch to using the run_to_rel_pos command.
...
# done: . Make sure the beep happens AFTER the motors stop.  Use the wait_while command to block code execution.

# done. Formally test your work. When you think you have the problem complete run these tests:
...
# done Call over a TA or instructor to sign your team's checkoff sheet and do a code review.
#
...
def main():
    ...
    while time_s != 0:
        ...
        if sp == 0:
            break
        d = int(input("Distance to travel (inches): "))
        if d == 0:
            break
        deg = convert_in_to_deg(d, sp)
        left_motor.run_to_rel_pose(position_sp = deg, speed_sp=sp, stop_action='brake')
        right_motor.run_to_rel_pos(position_sp = deg, speed_sp=sp, stop_action='brake')
        left_motor.wait_while(ev3.Motor.STATE_RUNNING)
        left_motor.wait_while(ev3.Motor.STATE.RUNNING)
        ev3.Sound.beep().wait()

    print("Goodbye!")
    ev3.Sound.speak("Goodbye").wait()

def convert_in_to_deg(d, sp):
    time_to_move = d/(sp*0.0116)
    deg = sp * time_to_move
    return deg
